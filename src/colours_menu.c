...
/*   Updated: 2023/01/22 20:41:03 by yugurlu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	colour(t_fdf *data, float z_start, float z_end)
{
	...
	else if (!ft_strchr(data->colour_matrix[(int)data->l->l_y_e]
			[(int)data->l->l_x_e], ','))
	{
		colour = ft_split(data->colour_matrix[(int)data->l->l_y_e]
			[(int)data->l->l_x_e], ',');
		data->colour = hex_to_decimal(colour[1]);
		free_split(colour);
	}
	else
		def_colour(data, z_start, z_end);
}
